...
module.exports = async (helper) => {
  try {
    ...
    if (!exists) {
      helper.fail(`
        JavaScript 코드 폴더에서 "freightTransformer.js" 스크립트를 찾을 수 없습니다. 
    ...
    if (tq.error) {
      ...
      if (tq.error.name === "ReferenceError") {
        return helper.fail(`
          코드 내에 <span class="highlight">transform</span> 
          함수가 정의되지 않은 것 같습니다. 스크립트의 전역(global) 스코프에서 
          해당 함수를 찾을 수 없었습니다.
          <br/><br/>
          함수 이름을 "<span class="highlight">transform</span>"(으)로 
    ...
    if (!isFunction(tq.transform)) {
      let message = `
        <span class="highlight">transform</span>이라는 이름의 변수는 
    ...
    try {
      ...
      if (!Array.isArray(result)) {
        return helper.fail(`
          ...
          함수는 변환된 문자열들이 담긴 배열을 반드시 반환해야 합니다.
        `);
      }
      ...
      return helper.fail(`
        transform 함수를 실행하는 중 오류가 발생했습니다. 명령줄(터미널)에서
    ...
    helper.success(`
      ...
      <span class="highlight">네 번째 문자</span>에 대한 인덱스가 
      나타났습니다:<br/><br/>
