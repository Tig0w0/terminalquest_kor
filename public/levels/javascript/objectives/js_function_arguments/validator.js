...
module.exports = async helper => {
  try {
    ...
    if (!exists) {
      helper.fail(`
        JavaScript 코드 폴더에서 "politeLasers.js" 스크립트를 찾을 수 없습니다. 
    ...
    try {
      ...
      if (result !== 'ON') {
        return helper.fail(`
          ...
          올바른 값은 <span class="highlight">"ON"</span>이어야 합니다.
        `);
      }
      ...
      if (result2 !== 'ON') {
        return helper.fail(`
          함수가 문자열을 반환하긴 했지만, 우리가 기대한 값이 아닙니다.
          함수가 반환한 값은 <span class="highlight">"${result2}"</span>입니다.
          올바른 값은 <span class="highlight">"ON"</span>이어야 합니다.
        `);
      }
      ...
      if (result3 !== 'OFF') {
        return helper.fail(`
          여러분의 함수에 "please"라는 문자열이 인수로 전달되면, 
          "OFF" 문자열을 반환해야 합니다.
        `);
      }
    ...
    helper.success(`
      깊은 한숨을 내쉬며, 여러분은 다시 한번 정중하게 요청할 때만 레이저 장벽을
      내리는 함수를 만들어 글렌을 달래줍니다. 이제 연구소 안쪽으로 더 깊이 
      들어갈 수 있습니다.
    `);
  } catch (e) {
    helper.fail(`
