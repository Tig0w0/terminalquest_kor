...
module.exports = async helper => {
  try {
    ...
    if (!exists) {
      helper.fail(`
        JavaScript 코드 폴더에서 "freightScanner.js" 스크립트를 찾을 수 없습니다. 
    ...
    if (tq.error) {
      ...
      if (tq.error.name === 'ReferenceError') {
        return helper.fail(`
          코드 내에 <span class="highlight">scan</span> 
          함수가 정의되지 않은 것 같습니다. 스크립트의 전역(global) 스코프에서 
          해당 함수를 찾을 수 없었습니다.
          <br/><br/>
          함수 이름을 "<span class="highlight">scan</span>"(으)로 
    ...
    if (!isFunction(tq.scan)) {
      let message = `
        <span class="highlight">scan</span>이라는 이름의 변수는 
    ...
    try {
      ...
      if (isNaN(result)) {
        return helper.fail(`
          ...
          함수는 입력받은 배열에서 "contraband" 문자열이 나타난 횟수를 
          나타내는 숫자를 반드시 반환해야 합니다.
        `);
      }

      if (result !== 0 || result2 !== 3 || result3 !== 2) {
        return helper.fail(`
          함수가 숫자를 반환하긴 했지만, 우리가 기대한 값이 아닙니다. 
          함수는 입력받은 배열 안의 모든 항목을 검사하여, "contraband"라는 
          문자열이 등장한 총 횟수를 반환해야 합니다.
        `);
      }

    } catch(ee) {
      return helper.fail(`
        scan 함수를 실행하는 중 오류가 발생했습니다. 명령줄(터미널)에서
    ...
    helper.success(`
      ...
      <span class="highlight">첫 번째 문자</span>에 대한 인덱스가 
      나타났습니다:<br/><br/>
