...
module.exports = async (helper) => {
  try {
    ...
    if (!exists) {
      helper.fail(`
        JavaScript 코드 폴더에서 "freightMass.js" 스크립트를 찾을 수 없습니다. 
    ...
    const testCode = `
      ...
      try {
        __TQ.calculateMass = calculateMass;
      } catch(e) {
        __TQ.error = e;
      }
    `;
    ...
    if (tq.error) {
      ...
      if (tq.error.name === "ReferenceError") {
        return helper.fail(`
          코드 내에 <span class="highlight">calculateMass</span> 
          함수가 정의되지 않은 것 같습니다. 스크립트의 전역(global) 스코프에서 
          해당 함수를 찾을 수 없었습니다.
          <br/><br/>
          함수 이름을 "<span class="highlight">calculateMass</span>"(으)로 
    ...
    if (!isFunction(tq.calculateMass)) {
      let message = `
        <span class="highlight">calculateMass</span>이라는 이름의 변수는 
    ...
    // Check functionality
    try {
      const result = tq.calculateMass([]);
      const result2 = tq.calculateMass(["farts", "burps", "toots"]);
      ...
      if (isNaN(result)) {
        return helper.fail(`
          여러분의 함수가 숫자를 반환하지 않는 것 같습니다. 여러분의 
          함수는 입력 배열에 있는 모든 문자열의 전체 길이를 나타내는 
          숫자를 반드시 반환해야 합니다.
        `);
      }

      if (result !== 0 || result2 !== 15) {
        return helper.fail(`
          함수가 숫자를 반환하긴 했지만, 우리가 기대한 값이 아닙니다. 
          도움말과 과제 탭을 다시 확인하여 올바른 숫자 값을 반환하는지 
          확인하세요.
        `);
      }
    } catch (ee) {
      return helper.fail(`
        calculateMass 함수를 실행하는 중 오류가 발생했습니다. 명령줄(터미널)에서
    ...
    helper.success(`
      ...
      <span class="highlight">다섯 번째 문자</span>에 대한 인덱스가 
      나타났습니다:<br/><br/>
      <span class="highlight">[0][4]</span>
      <br/><br/>
