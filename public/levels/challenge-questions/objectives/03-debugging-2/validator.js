...
module.exports = async function (helper) {
  try {
    const context = await helper.pullVarsFromQuestIdeUserCodeLocalScope(["sumRadLevels"], "03-debugging-2");
    assert(context.sumRadLevels, "sumRadLevels 함수가 정의되지 않았습니다!");
    const test = assertTestCase(context.sumRadLevels);
    test([
