...
module.exports = async function (helper) {
  try {
    const context = await helper.pullVarsFromQuestIdeUserCodeLocalScope(["removeDuplicateCharacters"], "remove-duplicate-characters");
    assert(context.removeDuplicateCharacters, "removeDuplicateCharacters 함수가 정의되지 않았습니다!");
    const test = assertTestCase(context.removeDuplicateCharacters);
    test("aabbcc", "abc"); test("aba", "aba");
